    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-4xl">
        <div className="animate-fade-in-up">
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Admissions Portal</h1>
          <p className="text-sm text-muted-foreground mt-1">Live patient admission — updates dashboard in real-time</p>
        </div>

        {/* Form */}
        <form onSubmit={handleSubmit} className="rounded-xl border border-border bg-card p-6 space-y-5 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
        ...
        <div className="rounded-xl border border-border bg-card p-5 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
          ...
          {admissions.length === 0 ? (
            <p className="text-sm text-muted-foreground py-8 text-center">No live admissions yet. Submit the form above to see real-time updates.</p>
          ) : (
            <div className="space-y-2">
              {admissions.slice(0, 10).map(a => (
                <div key={a.id} className="flex items-center justify-between rounded-lg bg-secondary/50 px-4 py-3 text-sm">
                  <div className="flex items-center gap-3">
                    <div className={`h-2 w-2 rounded-full ${a.severity >= 3 ? 'bg-destructive' : a.severity >= 2 ? 'bg-warning' : 'bg-success'}`} />
                    <span className="font-medium text-foreground">{a.patient_name}</span>
                  </div>
                  <span className="text-muted-foreground">{a.department}</span>
                  <span className="text-muted-foreground text-xs">{a.doctor_name}</span>
